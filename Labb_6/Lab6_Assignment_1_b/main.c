...
//*****************************************************************************
//
// Global semaphore handlers.
//
//*****************************************************************************
SemaphoreHandle_t semMutex; // Ensures exclusive access to the shared 'Data' struct.
SemaphoreHandle_t semFull; // Keeps track of available space, blocking the producer when full.
SemaphoreHandle_t semEmpty; // Keeps track of available space, blocking the consumer when empty.
...
void Producer(void* parameters)
{
    while(1)
    {
        if(xSemaphoreTake(semEmpty, portMAX_DELAY) == pdTRUE)
        {
            if(xSemaphoreTake(semMutex, portMAX_DELAY) == pdTRUE)
            {
        ...
        {
            UARTprintf("Waiting for empty slots.");
        }
...
void Consumer(void* parameters)
{
    while(1)
    {
        ...
        {
            UARTprintf("Waiting for item to be consumed.");
        }
...
int main(void)
{
    ...
    semFull = xSemaphoreCreateCounting(BUFFER_SIZE, 0);
